@include('common/header_link')
    ...
    <div class="bog_content">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="tennis-featured-card" id="featured-tennis-game">
                        <div class="featured-top">
                            <div>
                                <span class="featured-label">Featured Tennis</span>
                                <h3 id="featured-tennis-title">Loading top tennis match...</h3>
                                <p id="featured-tennis-league" class="featured-meta">Premium match highlight</p>
                            </div>
                            <div class="featured-status" id="featured-tennis-status">UPCOMING</div>
                        </div>
                        <div class="featured-details">
                            <div class="featured-team-block">
                                <strong id="featured-tennis-home">Player A</strong>
                                <span id="featured-tennis-home-score">0</span>
                            </div>
                            <div class="featured-vs">vs</div>
                            <div class="featured-team-block">
                                <strong id="featured-tennis-away">Player B</strong>
                                <span id="featured-tennis-away-score">0</span>
                            </div>
                        </div>
                        <div class="featured-meta-row">
                            <span id="featured-tennis-time">Match time</span>
                            <span id="featured-tennis-market">Top market: Match Winner</span>
                        </div>
                    </div>
...
.tennis-featured-card { background: linear-gradient(180deg, rgba(7,17,38,0.74), rgba(7,17,38,0.64)), url('{{ asset('assets/front_end/images/football_color.png') }}') center/cover no-repeat; border: 1px solid rgba(255,255,255,0.18); border-radius: 16px; padding: 22px; margin-bottom: 18px; box-shadow: 0 20px 36px rgba(0,0,0,0.16); position: relative; overflow: hidden; color: #fff; }
.tennis-featured-card::after { content: ''; position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(255,255,255,0.18), transparent 22%), radial-gradient(circle at 85% 15%, rgba(255,255,255,0.12), transparent 18%); opacity: 0.48; pointer-events: none; }
...
$(function(){
    ...
    function formatOdds(value) { return parseFloat(value).toFixed(2); }

    function buildFallbackOdd(market, selection) {
        var value = 1.80;
        if(market === 'match_winner') {
            if(selection === '1') value = 1.90; else if(selection === '2') value = 1.95;
        }
        if(market === 'handicap') value = 1.90;
        if(market === 'totals') value = selection && selection.indexOf('O') === 0 ? 1.75 : 2.05;
    ...
    function renderFixtures(fixtures, containerId) {
        ...
        $container.empty();
        if(!fixtures || fixtures.length === 0) { $container.append('<p style="text-align:center;color:#999;padding:30px;">No matches available</p>'); return; }
        ...
        fixtures.forEach(function(fixture){
            ...
            teamsDiv.append('<div class="fixture-team-pair">' + (fixture.home_player||fixture.home_team||'Player A') + ' v ' + (fixture.away_player||fixture.away_team||'Player B') + '</div>');
            ...
            card.append(renderMarketButtons(fixture, 'match_winner', 'Match Winner', [
                { label: fixture.home_player||fixture.home_team||'Player A', selection: '1' },
                { label: fixture.away_player||fixture.away_team||'Player B', selection: '2' }
            ]));

            card.append(renderMarketButtons(fixture, 'handicap', 'Handicap', [
                { label: fixture.home_player + ' -1.5', selection: '1' },
                { label: fixture.away_player + ' +1.5', selection: '2' }
            ]));

            card.append(renderMarketButtons(fixture, 'totals', 'Total Games Over/Under', [
                { label: 'O20.5', selection: 'O20.5' },
                { label: 'U20.5', selection: 'U20.5' },
                { label: 'O22.5', selection: 'O22.5' },
                { label: 'U22.5', selection: 'U22.5' }
            ]));
    ...
    function renderFeaturedGame(fixture) {
        if(!fixture) {
            $('#featured-tennis-title').text('No featured match available');
            $('#featured-tennis-league').text('Check back soon for premium fixtures');
            $('#featured-tennis-status').text('UPCOMING');
            $('#featured-tennis-home').text('Player A');
            $('#featured-tennis-away').text('Player B');
            $('#featured-tennis-home-score').text('-');
            $('#featured-tennis-away-score').text('-');
            $('#featured-tennis-time').text('Awaiting next highlight');
            $('#featured-tennis-market').text('Top market: Match Winner');
            return;
        }

        $('#featured-tennis-title').text((fixture.home_player||fixture.home_team) + ' vs ' + (fixture.away_player||fixture.away_team));
        $('#featured-tennis-league').text((fixture.league||'') + ' featured match');
        $('#featured-tennis-status').text(fixture.status === 'live' ? 'LIVE' : fixture.status === 'finished' ? 'FINISHED' : 'UPCOMING');
        $('#featured-tennis-home').text(fixture.home_player||fixture.home_team||'Player A');
        $('#featured-tennis-away').text(fixture.away_player||fixture.away_team||'Player B');
        $('#featured-tennis-home-score').text(fixture.status !== 'upcoming' ? (fixture.home_score || 0) : '-');
        $('#featured-tennis-away-score').text(fixture.status !== 'upcoming' ? (fixture.away_score || 0) : '-');
        $('#featured-tennis-time').text(fixture.status === 'upcoming' ? 'Start: ' + formatDate(fixture.kickoff_time || fixture.start_time) : 'Started: ' + formatDate(fixture.kickoff_time || fixture.start_time));
        $('#featured-tennis-market').text('Top market: Match Winner');
    }

    function fetchAndRenderAll() {
        $.getJSON('{{ url('api/fixtures') }}', {sport: 'tennis', status: 'all'}, function(data) {
        ...
        $.getJSON('{{ url('api/bet/slip') }}', function(data) {
            ...
            if(data.count === 0) {
                ...
                data.bets.forEach(function(bet) {
                    ...
                    info.append('<div class="slip-item-match">' + (bet.fixture.home_player||bet.fixture.home_team) + ' v ' + (bet.fixture.away_player||bet.fixture.away_team) + '</div>');
    ...
    $('body').on('click', '.odd-btn', function() {
        ...
        $.ajax({
            ...
            error: function() { swal('Error', 'Please login first', 'error'); }
        });
    });
    ...
    $('body').on('click', '.slip-item-remove', function() {
        ...
        $.ajax({ url: '{{ url('api/bet/remove') }}', type: 'POST', headers: {'X-CSRF-TOKEN': csrfToken}, data: {bet_id: betId}, success: function(){ updateSlip(); } });
    });

    // Clear slip
    $('#clear-slip-btn').on('click', function(e) { e.preventDefault(); $.getJSON('{{ url('api/bet/slip') }}', function(data) { data.bets.forEach(function(bet){ $.ajax({ url: '{{ url('api/bet/remove') }}', type: 'POST', headers: {'X-CSRF-TOKEN': csrfToken}, data: {bet_id: bet.id}, async: false }); }); updateSlip(); }); });

    // Submit slip
    $('#submit-slip-btn').on('click', function(e) { e.preventDefault(); $.ajax({ url: '{{ url('api/bet/submit') }}', type: 'POST', headers: {'X-CSRF-TOKEN': csrfToken}, success: function(){ swal('Success!', 'Your bet slip has been submitted', 'success'); setTimeout(function(){ updateSlip(); }, 800); }, error: function(){ swal('Error', 'Failed to submit bets', 'error'); } }); });
