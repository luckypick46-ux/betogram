@include('common/header_link')
    ...
    <div class="bog_content">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="page-header" style="margin-bottom: 24px; padding: 18px 22px; background: #f4f7fb; border: 1px solid #d7e0ed; border-radius: 6px;">
                        <h2 style="margin: 0; font-size: 24px; color: #1a3f72;"><i class="fa fa-basketball-ball"></i> Basketball Betting</h2>
                        <p style="margin: 8px 0 0; color: #5a6371; font-size: 14px; max-width: 720px;">Get fast access to basketball game markets with clear status panels for upcoming, live and finished matchups.</p>
                    </div>
                ...
                <div class="col-md-3">
                    <div class="betingtab" style="position: sticky; top: 90px;">
                        <h3 style="padding: 15px; margin: 0; border-bottom: 1px solid #c7d4e2; display: flex; justify-content: space-between; align-items: center;">
                            <span><i class="fa fa-list"></i> Bet Slip</span>
                            <span id="slip-count" class="badge" style="background: #1a3f72; color: #fff; padding: 4px 8px; border-radius: 3px; font-size: 11px; font-weight: 600;">0</span>
                        </h3>
                        ...
                        <div style="padding: 15px; border-top: 1px solid #c7d4e2; background: #f8faff;">
                            ...
                            <div class="slip-summary-item" style="border-top: 1px solid #dce4ee; padding-top: 10px; margin-top: 10px;">
                                <span style="font-weight: 600;">Potential Return:</span>
                                <strong id="potential-return" style="color: #1a3f72; font-size: 16px;">$0.00</strong>
                            </div>
                            ...
                            <div style="margin-top: 15px;">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" id="stake-input" class="form-control" value="10" min="1" max="10000" step="5" style="border: 1px solid #b8c7d6; height: 40px; border-radius: 0 2px 2px 0;">
                                </div>
                            </div>

                            <div style="margin-top: 12px;">
                                <a href="#" id="submit-slip-btn" class="btn btn-deposit btn-block" style="background: #1a3f72; color: #fff; padding: 12px; border-radius: 4px; text-decoration: none; font-weight: 600; text-align: center; display: inline-block; width: 100%; box-sizing: border-box; border: none; margin-bottom: 8px; transition: all 0.2s ease;">Place Bets</a>
                                <a href="#" id="clear-slip-btn" class="btn btn-default btn-block" style="background: #e7edf5; color: #333; padding: 10px; border-radius: 4px; text-decoration: none; font-weight: 600; text-align: center; display: inline-block; width: 100%; box-sizing: border-box; border: 1px solid #c7d4e2; transition: all 0.2s ease;">Clear Slip</a>
                            </div>
                        </div>
                    </div>
                </div>
...
.fixture-card-item { background: #fff; border: 1px solid #d7e0ed; border-radius: 6px; padding: 18px; display: flex; flex-direction: column; box-shadow: 0 2px 6px rgba(26, 63, 114, 0.08); transition: all 0.2s ease; }
.fixture-card-item:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(26, 63, 114, 0.12); }
.fixture-league-label { font-size: 11px; color: #6c7a92; text-transform: uppercase; font-weight: 700; margin-bottom: 10px; }
.fixture-match-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; }
.fixture-teams { flex: 1; }
.fixture-team-pair { font-weight: 700; color: #1a3f72; font-size: 15px; line-height: 22px; margin-bottom: 6px; }
.fixture-time-info { font-size: 12px; color: #677283; }
.fixture-score-info { font-size: 14px; font-weight: 700; color: #222f47; margin-top: 4px; }
...
.fixture-status.upcoming { background: #1a3f72; color: #fff; }
.fixture-market { margin-top: 16px; }
.fixture-market-title { font-size: 12px; color: #1a3f72; font-weight: 700; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.4px; }
.fixture-odds-grid { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px; }
.odd-btn { flex: 1 1 120px; min-width: 120px; padding: 10px 8px; background: #f6f8fb; border: 1px solid #d5dae2; border-radius: 4px; text-align: center; cursor: pointer; transition: all 0.2s ease; font-size: 13px; }
.odd-btn:hover { background: #e9f2ff; border-color: #1a3f72; }
.odd-btn.selected { background: #1a3f72; color: #fff; border-color: #1a3f72; }
.odd-label { font-size: 11px; color: #5b6576; margin-bottom: 4px; }
.odd-value { font-weight: 700; color: #1a3f72; font-size: 14px; }
.odd-btn.selected .odd-label { color: #cbdcff; }
...
.slip-content { border-top: 1px solid #c7d4e2; background: #f8faff; }
.slip-item { display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid #dce4ee; background: #fff; }
.slip-item-info { flex: 1; }
.slip-item-match { font-size: 12px; font-weight: 700; color: #1a3f72; }
.slip-item-odd { font-size: 11px; color: #677283; }
.slip-item-remove { cursor: pointer; color: #d9534f; font-weight: 700; font-size: 16px; line-height: 1; }
#submit-slip-btn:hover { background: #122f57 !important; }
#clear-slip-btn:hover { background: #d4dce6 !important; }
...
$(function(){
    ...
    function buildFallbackOdd(fixture, market, selection) {
        ...
        if(market === '1x2') {
            if(selection === '1') value = 1.85;
            else if(selection === '2') value = 1.95;
        }

        if(market === 'totals') {
            var threshold = parseFloat(selection.slice(1));
            if(selection[0] === 'O') {
                value = 1.60 + (threshold * 0.007);
            } else {
                value = 1.42 + (threshold * 0.006);
            }
        }

        if(market === 'spread') {
            value = selection.indexOf('-') === 0 ? 1.90 : 1.95;
        }
    ...
    function renderFixtures(fixtures, containerId) {
        ...
        fixtures.forEach(function(fixture) {
            ...
            card.append(renderMarketButtons(fixture, 'totals', 'Total Points Over/Under', [
                { label: 'O205.5', selection: 'O205.5' },
                { label: 'U205.5', selection: 'U205.5' },
                { label: 'O215.5', selection: 'O215.5' },
                { label: 'U215.5', selection: 'U215.5' }
            ]));
    ...
    function fetchAndRenderAll() {
        $.getJSON('{{ url('api/fixtures') }}', {sport: 'basketball', status: 'all'}, function(data) {
