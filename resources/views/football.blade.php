@include('common/header_link')
    ...
    <div class="bog_content">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="football-featured-card" id="featured-football-game">
                        <div class="featured-top">
                            <div>
                                <span class="featured-label">Featured Match</span>
                                <h3 id="featured-football-title">Loading top football game...</h3>
                                <p id="featured-football-league" class="featured-meta">Premium matchday highlight</p>
                            </div>
                            <div class="featured-status" id="featured-football-status">UPCOMING</div>
                        </div>
                        <div class="featured-details">
                            <div class="featured-team-block">
                                <strong id="featured-football-home">Home Team</strong>
                                <span id="featured-football-home-score">0</span>
                            </div>
                            <div class="featured-vs">vs</div>
                            <div class="featured-team-block">
                                <strong id="featured-football-away">Away Team</strong>
                                <span id="featured-football-away-score">0</span>
                            </div>
                        </div>
                        <div class="featured-meta-row">
                            <span id="featured-football-time">Kickoff information</span>
                            <span id="featured-football-market">Top market: Match Result</span>
                        </div>
                    </div>
...
.football-featured-card::after { content: ''; position: absolute; inset: 0; background-image: radial-gradient(circle at 20% 20%, rgba(255,255,255,0.18), transparent 22%), radial-gradient(circle at 85% 15%, rgba(255,255,255,0.12), transparent 18%); opacity: 0.48; pointer-events: none; }
...
$(function(){
    ...
    function buildFallbackOdd(fixture, market, selection) {
        ...
        if(market === 'btts') {
            value = selection === 'Yes' ? 1.75 : 2.10;
        }

        if(market === 'ht_score') {
            if(selection === '0-0') value = 4.20;
            if(selection === '1-0') value = 4.60;
            if(selection === '0-1') value = 5.00;
            if(selection === '1-1') value = 5.50;
        }

        if(market === 'ft_score') {
            if(selection === '1-0') value = 6.10;
            if(selection === '0-1') value = 7.20;
            if(selection === '1-1') value = 5.80;
            if(selection === '2-1') value = 8.50;
            if(selection === '1-2') value = 9.10;
            if(selection === '2-2') value = 12.00;
        }
    ...
    function renderFixtures(fixtures, containerId) {
        ...
        fixtures.forEach(function(fixture) {
            ...
            teamsDiv.append('<div class="fixture-team-pair">' + fixture.home_team + ' v ' + fixture.away_team + '</div>');
            ...
            card.append(renderMarketButtons(fixture, '1x2', 'Match Result', [
                { label: fixture.home_team, selection: '1' },
                { label: 'Draw', selection: 'X' },
                { label: fixture.away_team, selection: '2' }
            ]));

            card.append(renderMarketButtons(fixture, 'totals', 'Total Goals Over/Under', [
                { label: 'O0.5', selection: 'O0.5' },
                { label: 'U0.5', selection: 'U0.5' },
                { label: 'O1.5', selection: 'O1.5' },
                { label: 'U1.5', selection: 'U1.5' },
                { label: 'O2.5', selection: 'O2.5' },
                { label: 'U2.5', selection: 'U2.5' },
                { label: 'O3.5', selection: 'O3.5' },
                { label: 'U3.5', selection: 'U3.5' },
                { label: 'O4.5', selection: 'O4.5' },
                { label: 'U4.5', selection: 'U4.5' },
                { label: 'O5.5', selection: 'O5.5' },
                { label: 'U5.5', selection: 'U5.5' },
                { label: 'O6.5', selection: 'O6.5' },
                { label: 'U6.5', selection: 'U6.5' }
            ]));

            card.append(renderMarketButtons(fixture, 'btts', 'Both Teams To Score', [
                { label: 'Yes', selection: 'Yes' },
                { label: 'No', selection: 'No' }
            ]));

            card.append(renderMarketButtons(fixture, 'ht_score', 'Half Time Correct Score', [
                { label: '0-0', selection: '0-0' },
                { label: '1-0', selection: '1-0' },
                { label: '0-1', selection: '0-1' },
                { label: '1-1', selection: '1-1' }
            ]));

            card.append(renderMarketButtons(fixture, 'ft_score', 'Full Time Correct Score', [
                { label: '1-0', selection: '1-0' },
                { label: '0-1', selection: '0-1' },
                { label: '1-1', selection: '1-1' },
                { label: '2-1', selection: '2-1' },
                { label: '1-2', selection: '1-2' },
                { label: '2-2', selection: '2-2' }
            ]));
    ...
    function renderFeaturedGame(fixture) {
        if(!fixture) {
            $('#featured-football-title').text('No featured game available');
            $('#featured-football-league').text('Check back soon for premium fixtures');
            $('#featured-football-status').text('UPCOMING');
            $('#featured-football-home').text('Home');
            $('#featured-football-away').text('Away');
            $('#featured-football-home-score').text('-');
            $('#featured-football-away-score').text('-');
            $('#featured-football-time').text('Awaiting next highlight');
            $('#featured-football-market').text('Top market: Match Result');
            return;
        }

        $('#featured-football-title').text(fixture.home_team + ' vs ' + fixture.away_team);
        $('#featured-football-league').text(fixture.league + ' featured match');
        $('#featured-football-status').text(fixture.status === 'live' ? 'LIVE' : fixture.status === 'finished' ? 'FINISHED' : 'UPCOMING');
        $('#featured-football-home').text(fixture.home_team);
        $('#featured-football-away').text(fixture.away_team);
        $('#featured-football-home-score').text(fixture.status !== 'upcoming' ? (fixture.home_score || 0) : '-');
        $('#featured-football-away-score').text(fixture.status !== 'upcoming' ? (fixture.away_score || 0) : '-');
        $('#featured-football-time').text(fixture.status === 'upcoming' ? 'Kickoff: ' + formatDate(fixture.kickoff_time) : 'Started: ' + formatDate(fixture.kickoff_time));
        $('#featured-football-market').text('Top market: Match Result');
    }

    function fetchAndRenderAll() {
        $.getJSON('{{ url('api/fixtures') }}', {sport: 'football', status: 'all'}, function(data) {
